...
while (lenYT > 0) and count<maxcount:
   ...
   try:
      yt = YouTube(t, 'ANDROID', on_progress_callback = on_progress)
      yt_title = yt.title
      print(yt_title)
      ...
      new_file_name2 = new_file_name + '.mp3'
      ys.download(output_path=new_path, filename=new_file_name1)

      # convert mp4 to mp3
      subprocess.run(['ffmpeg','-i',os.path.join(new_path, new_file_name1) ,'-q:a','0','-map','a',os.path.join(new_path, new_file_name2),'-n'])
      os.remove(os.path.join(new_path, new_file_name1))
      ...
      print(t+"  ... TRY AGAIN, regex match error: "+t)
      print('====================================')
      j+=1
      lenYT -= 1
      continue
   except E.VideoUnavailable:
      print(t + "  Video unavailable: "+t)
      print('====================================')
      j+=1
      lenYT -= 1
      continue
   except E.LiveStreamError:
      print(t + " Live stream error: " + t)
      print('====================================')
      j+=1
      lenYT -= 1
      continue
